...
import { emptyDropdownCellValue } from "@mzawadie/components/Datagrid/DropdownCell";
import { numberCellEmptyValue } from "@mzawadie/components/Datagrid/NumberCell";
import {
    booleanCell,
    dropdownCell,
    moneyCell,
    numberCell,
    textCell,
} from "@mzawadie/components/Datagrid/cells";
import { AvailableColumn } from "@mzawadie/components/Datagrid/types";
import { DatagridChange, DatagridChangeOpts } from "@mzawadie/components/Datagrid/useDatagridChange";
...
import { ProductVariantListError } from "@mzawadie/pages/products/views/ProductUpdate/handlers/errors";
...
export function getVariantInputs(
    ...
    return variants
        ...
        .filter(
            (variables) =>
                variables.input.sku || variables.input.name || variables?.input?.attributes?.length > 0
        );
}
...
export function getStocks(
    ...
    return variants
        ...
        .filter((variables) => variables.removeStocks.length > 0 || variables.stocks.length > 0);
}
