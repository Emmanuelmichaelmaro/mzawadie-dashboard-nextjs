...
function findAttribute(attributes: CreateVariantInput, stockOrPrice: Stock | Price) {
    return attributes.find((attribute) => attribute.attributeId === stockOrPrice.attribute);
}

function getAttributeValueStock(attributes: CreateVariantInput, stock: Stock): number[] {
...
function getAttributeValuePrice(attributes: CreateVariantInput, price: Price): ChannelPrice[] {
...
function getStockFromMode(attributes: CreateVariantInput, stock: Stock, skipValue: number[]): number[] {
...
export function createVariants(data: ProductVariantCreateFormData): ProductVariantBulkCreateInput[] {
    if (
    ...
    const variants = createVariantFlatMatrixDimension([[]], data.attributes).map((variant) =>
        createVariant(data, variant)
    );
