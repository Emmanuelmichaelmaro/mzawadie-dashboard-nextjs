...
import { AttributeValue, ProductVariantCreateFormData, VariantCreatorPricesAndSkuMode } from "./form";
...
function selectValue(
    ...
    const attribute = prevState.attributes.find((attribute) => attribute.id === attributeId);
...
function changeAttributeValuePrice(
    ...
    const index = state.price.values.findIndex((value) => value.slug === attributeValueSlug);
...
function changeAttributeValueStock(
    ...
    const index = state.stock.values.findIndex((value) => value.slug === attributeValueSlug);
    ...
    const values = updateAtIndex(
        {
            ...
            value: updateAtIndex(quantity, state.stock.values[index].value, warehouseIndex),
        },
        state.stock.values,
        index
    );
...
function changeApplyPriceToAttributeId(
    ...
    const attribute = state.attributes.find((attribute) => attribute.id === attributeId);
...
function changeApplyPriceToAllValue(
    ...
    const channelIndex = prevChannels?.findIndex((channel) => channelId === channel.channelId);
...
function changeVariantStockData(
    ...
    variant.stocks = update(stock, variant.stocks, (a, b) => a.warehouse === b.warehouse);
...
function createVariantMatrix(state: ProductVariantCreateFormData): ProductVariantCreateFormData {
    return {
        ...state,
...
function reduceProductVariantCreateFormData(
    ...
    switch (action.type) {
        case ProductVariantCreateReducerActionType.selectValue:
            return selectValue(prevState, action.selectValue.attributeId, action.selectValue.value);
