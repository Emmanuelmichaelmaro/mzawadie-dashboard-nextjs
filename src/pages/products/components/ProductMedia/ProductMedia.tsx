...
import { ProductMediaFragment, ProductMediaType } from "@mzawadie/graphql";
import { ProductMediaPopper } from "@mzawadie/pages/products/components/ProductMediaPopper/ProductMediaPopper";
import createMultiFileUploadHandler from "@mzawadie/utils/handlers/multiFileUploadHandler";
...
const SortableMedia = SortableElement<SortableMediaProps>(({ media, onEdit, onDelete }) => (
    <MediaTile media={media} onEdit={onEdit ? () => onEdit(media.id) : undefined} onDelete={onDelete} />
));
    ...
    ({ media, preview, onDelete, onEdit, ...props }) => (
        <div {...props}>
            ...
            {preview
                ...
                .map((mediaObj, index) => (
                    <MediaTile loading media={mediaObj} key={index} />
                ))}
        </div>
    )
);
