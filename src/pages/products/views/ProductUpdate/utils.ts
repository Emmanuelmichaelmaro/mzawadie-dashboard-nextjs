...
import { ChannelsWithVariantsData, ChannelVariantListing, ChannelWithVariantData } from "./types";
...
export const getChannelVariantToggleData = (variants: string[], isSelected: boolean): string[] =>
    isSelected ? [] : variants;
...
export const areAllChannelVariantsSelected = (variants: string[] = [], selectedVariants: string[]) =>
    selectedVariants.length === variants.length;

export const areAnyChannelVariantsSelected = (channelsWithVariantsData: ChannelWithVariantData) =>
    channelsWithVariantsData?.selectedVariantsIds.length > 0;

export const getTotalSelectedChannelsCount = (channelsWithVariantsData: ChannelsWithVariantsData) =>
    Object.values(channelsWithVariantsData).filter((channel) => channel.selectedVariantsIds.length > 0)
        .length;
...
export const addAllVariantsToAllChannels = (
    ...
    const areAllChannelsSelected = areAllVariantsAtAllChannelsSelected(variants, listings);
...
export const channelVariantListingDiffToDict = (
    ...
    reduce(
        listing,
        (listingDict: ChannelVariantListing, { selectedVariantsIds }, channelId) => ({
            ...listingDict,
