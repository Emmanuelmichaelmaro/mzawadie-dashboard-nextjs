...
const OrderFulfillPage: React.FC<OrderFulfillPageProps> = (props) => {
    const { loading, errors, order, saveButtonBar, warehouses, shopSettings, onBack, onSubmit } = props;

    const intl = useIntl();
    const classes = useStyles(props);

    const { change: formsetChange, data: formsetData } = useFormset<null, OrderFulfillStockInput[]>(
    ...
    const shouldEnableSave = () => {
        ...
        const areProperlyFulfilled = formsetData?.every(({ id, value }) => {
            ...
            const { quantityToFulfill } = lines.find(({ id: lineId }) => lineId === id);

            const formQuantityFulfilled = value?.reduce((result, { quantity }) => result + quantity, 0);
