...
export function mapAsyncEventsToChoices(
    ...
    const isAnyAsyncEventSelected = selectedEvents.includes(WebhookEventTypeAsyncEnum.ANY_EVENTS);

    return events.map((event) => ({
        label: event,
        value: event,
        disabled: event !== WebhookEventTypeAsyncEnum.ANY_EVENTS && isAnyAsyncEventSelected,
    }));
}
