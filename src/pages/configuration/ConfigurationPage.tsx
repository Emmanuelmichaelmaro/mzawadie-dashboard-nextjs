...
export const ConfigurationPage: React.FC<ConfigurationPageProps> = (props) => {
    ...
    return (
        <Container>
            ...
            {menus
                .filter((menu) =>
                    menu.menuItems.some((menuItem) => hasAnyPermissions(menuItem.permissions, user))
                )
                .map((menu, menuIndex) => (
                    <div className={classes.configurationCategory} key={menuIndex}>
                        ...
                        <div className={classes.configurationItem}>
                            {menu.menuItems
                                .filter((menuItem) => hasAnyPermissions(menuItem.permissions, user))
                                .map((item, itemIndex) => (
                                    <Link className={classes.link} to={item.url}>
                                        <NavigationCard
                                            ...
                                            data-test-id={`${
                                                item.testId
                                            }-settings-subsection-${item.title.toLowerCase()}`}
                                        />
                                    </Link>
                                ))}
                        </div>
