...
const sendResponseStatus = (actionId: string, ok: boolean): DispatchResponseEvent => ({
    type: "response",
    payload: {
        actionId,
        ok,
...
export const useAppActions = (
    ...
    const actionReducer = (action: Actions | undefined): DispatchResponseEvent => {
        switch (action?.type) {
            case "notification": {
                const { actionId, ...notification } = action.payload as NotificationAction["payload"];
            ...
            case "redirect": {
                const { to, newContext, actionId } = action.payload as RedirectAction["payload"];

                let success = true;

                const appDeepUrlChange = isAppDeepUrlChange(appId, location.pathname, to);
                ...
                try {
                    if (newContext) {
                        ...
                        const isExternalDomain = new URL(to).hostname !== window.location.hostname;
