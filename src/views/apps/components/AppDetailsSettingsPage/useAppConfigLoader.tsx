...
// @ts-nocheck
import { AppFragment } from "@mzawadie/fragments/types/AppFragment";
import { useEffect, useRef } from "react";
import urlJoin from "url-join";

export type UseAppConfigLoaderCallbacks = Record<"onLoad" | "onError", () => void>;
    ...
    if (!frameContainer?.innerHTML && data?.configurationUrl) {
        try {
            const response = await fetch(data?.configurationUrl, {
                headers: {
                    "x-mzawadie-domain": backendHostname,
                    "x-mzawadie-token": data.accessToken,
                },
                method: "GET",
            });
