...
const VariantDetailsChannelsAvailabilityCard: React.FC<VariantDetailsChannelsAvailabilityCardProps> = ({
    ...
    const getProductChannelListingByChannelId = (channelId: string) =>
        variant?.product.channelListings.find(({ channel }) => channel.id === channelId);

    const getItemSubtitle = (channelId: string) => {
        const { isPublished, publicationDate } = getProductChannelListingByChannelId(channelId);
    ...
    const variantChannelListingsChannelsIds = channelListings.map(({ channel: { id } }) => id);

    const allAvailableChannelsListings = variant.product.channelListings.filter(({ channel }) =>
