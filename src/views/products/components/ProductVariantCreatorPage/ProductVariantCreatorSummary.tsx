...
import { ProductVariantBulkCreate_productVariantBulkCreate_errors } from "@mzawadie/views/products/types/ProductVariantBulkCreate";
    ...
    (theme) => ({
        ...
        summary: {
            ...
            gridTemplateColumns: (props) =>
                `minmax(180px, auto) repeat(${
                    props.data.variants[0].channelListings.length + props.data.variants[0].stocks.length
                }, minmax(180px, auto)) 140px 64px`,
            overflowX: "scroll",
...
function getVariantName(variant: ProductVariantBulkCreateInput, attributes: Attribute[]): string[] {
    ...
    return (
        <Card className={classes.card}>
            ...
            <div className={classes.summary}>
                <div className={classNames(classes.col, classes.colHeader, classes.colName)}>
                    <FormattedMessage defaultMessage="Variant" id="V76IV7" description="variant name" />
                </div>

                {channelListings.map((listing) => (
                    <div
                        key={listing.id}
                        className={classNames(classes.col, classes.colHeader, classes.colPrice)}
                    >
                        <FormattedMessage
                ...
                {data.warehouses.map((warehouseId) => (
                    <div
                        className={classNames(classes.col, classes.colHeader, classes.colStock)}
                        key={warehouseId}
                    >
                        {warehouses.find((warehouse) => warehouse.id === warehouseId).name}
                    </div>
                ))}
                <div className={classNames(classes.col, classes.colHeader, classes.colSku)}>
                    <FormattedMessage defaultMessage="SKU" id="k4brJy" />
                </div>
                <div className={classNames(classes.col, classes.colHeader)} />
                <Hr className={classes.hr} />
                {data.variants.map((variant, variantIndex) => {
                    const variantErrors = errors.filter((error) => error.index === variantIndex);
                    ...
                    return (
                        ...
                        >
                            <div className={classNames(classes.col, classes.colName)}>
                                {getVariantName(variant, data.attributes).map((value, valueIndex) => (
                                    <span
                            ...
                            <div className={classNames(classes.col, classes.colSku)}>
                                <TextField
                                    ...
                                    helperText={getBulkProductErrorMessage(variantFormErrors.sku, intl)}
                                    fullWidth
                                    value={variant.sku}
                                    onChange={(event) =>
                            ...
                            {variantIndex !== data.variants.length - 1 && <Hr className={classes.hr} />}
                        </React.Fragment>
                    );
                })}
            </div>
