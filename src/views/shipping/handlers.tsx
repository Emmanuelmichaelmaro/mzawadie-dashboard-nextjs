...
// @ts-nocheck
import { commonMessages, getMutationErrors, getMutationState } from "@mzawadie/core";
...
import { FormData as ShippingZoneRatesPageFormData } from "@mzawadie/views/shipping/components/ShippingZoneRatesPage";
import { CreateShippingRateVariables } from "@mzawadie/views/shipping/types/CreateShippingRate";
import { ShippingMethodChannelListingUpdateVariables } from "@mzawadie/views/shipping/types/ShippingMethodChannelListingUpdate";
import { UpdateShippingRateVariables } from "@mzawadie/views/shipping/types/UpdateShippingRate";
...
const getPostalCodeRulesToAdd = (rules: ShippingMethodFragment_postalCodeRules[]) =>
    rules
        ...
        input: {
            addChannels:
                formChannels?.map((channel) => ({
                    ...
                    maximumOrderPrice: channel.maxValue && !noLimits ? channel.maxValue : null,
                    minimumOrderPrice: channel.minValue && !noLimits ? channel.minValue : null,
...
export function useShippingRateCreator(
    ...
    const [createBaseShippingRate, createBaseShippingRateOpts] = useShippingRateCreate({});
    ...
    const getUrl =
        type === ShippingMethodTypeEnum.PRICE ? shippingPriceRatesEditUrl : shippingWeightRatesEditUrl;
    ...
    const createShippingRate = async (data: ShippingZoneRatesPageFormData) => {
        ...
        if (createErrors?.length === 0) {
            ...
            if (mutationResults.find((result) => getMutationErrors(result.data as any).length > 0)) {
                deleteShippingRate({
                    variables: {
                        id: rateId,
                    },
