...
const FilterContentBody: React.FC<FilterContentBodyProps> = ({
    ...
    const isDateField = [FieldType.date, FieldType.dateTime].includes(filter.type);
    const isNumericField = [FieldType.price, FieldType.number].includes(filter.type);
        ...
        <div className={classes.filterSettings}>
            ...
            {filter.type === FieldType.boolean &&
                filter.options?.map((option) => (
                    <div className={classNames(classes.option, classes.optionRadio)} key={option.value}>
                        <FormControlLabel
                            control={
                                <Radio
